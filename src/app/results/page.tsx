...
import {
  MapPin, Star, Globe, Lock,
  ChevronDown, ChevronUp, Check, ArrowRight, Plane, Calendar, Key
} from 'lucide-react'
...
function CountryCard({
  ...
  return (
    ...
    >
      <div className={`p-6 cursor-pointer ${index === 0 ? 'bg-gold-400/5' : ''}`} onClick={() => setExpanded(!expanded)}>
              ...
              <ul className="space-y-2">
                {rec.pros.map(pro => (
                  <li key={pro} className="flex items-start gap-2 text-sm text-white/60">
                    <div className="w-1.5 h-1.5 rounded-full bg-jade-400 mt-1.5 shrink-0" />{pro}
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="text-red-400 text-xs font-mono uppercase tracking-wider mb-3">⚠ Challenges</h4>
              <ul className="space-y-2">
                {rec.cons.map(con => (
                  <li key={con} className="flex items-start gap-2 text-sm text-white/60">
                    <div className="w-1.5 h-1.5 rounded-full bg-red-400 mt-1.5 shrink-0" />{con}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {rec.visaOptions.map(v => (
                <span key={v} className="glass-gold text-xs px-3 py-1.5 rounded-full text-gold-300">{v}</span>
              ))}
            </div>
          </div>
          {rec.relocationPlan && (
            <div>
              <button onClick={() => setPlanExpanded(!planExpanded)} className="flex items-center gap-2 text-sm text-white/60 hover:text-white/80 transition-colors mb-3">
          ...
          {!isPremium && (
            <div className="relative rounded-2xl overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-obsidian/60 backdrop-blur-sm rounded-2xl">
                <button onClick={onUpgrade} className="flex items-center gap-2 glass-gold px-5 py-3 rounded-xl text-gold-300 text-sm font-semibold hover:bg-gold-400/10 transition-colors">
                  <Lock className="w-4 h-4" />Unlock Visa Steps, Housing & Tax Guide
                </button>
              </div>
            </div>
          )}
...
function UnlockModal({ onClose, onUnlock }: { onClose: () => void; onUnlock: () => void }) {
  const [code, setCode] = useState('')
  const [error, setError] = useState('')

  const handleSubmit = () => {
    if (code.trim().toLowerCase() === 'escapeusa') {
      onUnlock()
      onClose()
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6" onClick={onClose}>
      ...
      <div className="relative glass-gold rounded-3xl p-8 max-w-md w-full glow-gold" onClick={e => e.stopPropagation()}>
        <div className="text-center mb-6">
          ...
          <h3 className="font-display text-2xl font-bold mb-2">Enter Access Code</h3>
          <p className="text-white/50 text-sm">Enter your access code to unlock the full relocation plan</p>
        </div>
        <input
          type="text"
          value={code}
          ...
          className="w-full glass rounded-xl px-4 py-3.5 text-center text-lg font-mono tracking-widest bg-transparent border border-white/10 focus:border-gold-400/50 outline-none mb-3 placeholder:text-white/20"
          autoFocus
        />
        {error && <p className="text-red-400 text-sm text-center mb-3">{error}</p>}
        <button onClick={handleSubmit} className="btn-primary w-full py-3.5 rounded-xl font-bold text-base mb-3">
          Unlock Full Plan
        </button>
        <button onClick={onClose} className="w-full text-white/30 text-sm hover:text-white/50 transition-colors py-2">
          Cancel
        </button>
        <p className="text-white/20 text-xs text-center mt-4">Purchase at our store to receive your access code</p>
      </div>
    </div>
  )
}
...
function ResultsContent() {
  ...
  useEffect(() => {
    ...
    const unlocked = sessionStorage.getItem('exitplan_premium')
    if (unlocked === 'true' || upgraded) setIsPremium(true)
    setLoading(false)
  }, [])

  const handleUnlock = () => {
    setIsPremium(true)
    sessionStorage.setItem('exitplan_premium', 'true')
  }

  if (loading) return <div className="min-h-screen bg-obsidian flex items-center justify-center"><div className="text-white/40">Loading...</div></div>
  ...
  if (!recommendations.length) return (
    <div className="min-h-screen bg-obsidian flex items-center justify-center px-6 text-center">
      <div>
        ...
        <p className="text-white/40 mb-6">Let's start over.</p>
        <Link href="/quiz" className="btn-primary px-6 py-3 rounded-xl font-semibold">Take the Quiz</Link>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-obsidian">
      {showModal && <UnlockModal onClose={() => setShowModal(false)} onUnlock={handleUnlock} />}
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 py-5 max-w-5xl mx-auto border-b border-white/5">
        ...
        <div className="flex items-center gap-3">
          <Link href="/quiz" className="text-white/40 hover:text-white/70 text-sm transition-colors">Retake Quiz</Link>
          {!isPremium && (
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-6 py-12">
        <div className="text-center mb-12 animate-fade-up">
          ...
          <h1 className="font-display text-4xl md:text-5xl font-bold mb-3">Your Top <span className="gradient-text">3 Countries</span></h1>
          ...
          <div className="glass-gold rounded-3xl p-6 mb-10 flex flex-col md:flex-row items-center justify-between gap-4 glow-gold animate-fade-up">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-white/50 text-sm mt-0.5">Visa steps · Housing sites · Neighborhoods · Tax guide · Healthcare setup</p>
              </div>
            </div>
            <button onClick={() => setShowModal(true)} className="btn-primary shrink-0 flex items-center gap-2 px-7 py-3.5 rounded-xl font-bold whitespace-nowrap">
              Enter Access Code <Key className="w-4 h-4" />
            </button>
          </div>
        )}
        <div className="space-y-6">
          {recommendations.map((rec, i) => (
            <CountryCard key={rec.country} rec={rec} index={i} isPremium={isPremium} onUpgrade={() => setShowModal(true)} />
          ))}
        </div>
        {!isPremium && (
          <div className="mt-12 glass rounded-3xl p-8 text-center animate-fade-up">
            <h3 className="font-display text-2xl font-bold mb-4">Unlock the <span className="gradient-text-gold">Full Plan</span> for $19</h3>
            <p className="text-white/40 mb-6">Purchase at our store, get your code, enter it here — instant access.</p>
            <button onClick={() => setShowModal(true)} className="btn-primary inline-flex items-center gap-2 px-10 py-4 rounded-2xl font-bold text-lg">
              Enter Access Code <Key className="w-5 h-5" />
            </button>
            <p className="text-white/20 text-xs mt-3">One-time purchase · Instant access</p>
          </div>
        )}
        <p className="text-white/20 text-xs text-center mt-12">For informational purposes only. Consult an immigration attorney before relocating.</p>
      </main>
    </div>
  )
}
...
export default function ResultsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-obsidian flex items-center justify-center"><div className="text-white/40">Loading...</div></div>}>
      <ResultsContent />
    </Suspense>
  )
}
